...
Installiere das Modul mit folgendem Befehl, falls es nicht installiert ist. Aktiviere die virtuelle Umgebung (hier unter `~/.env`) und installiere das Modul^[In dieser Übung wird die Python Virtual Environment `.env` im Homeordner `~/` erstellt und aktiviert. Diese kann auch an einem Ort erstellt werden, beispielweise projektbasiert. Erstellen einer Virtual Environment: `python -m venv ~/Development/Projekt1/env`].
